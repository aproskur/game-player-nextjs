...
export const actionHandlers = {
    ...
    requestServer: (props, updateAppState) => {
        ...
        const entryPointKey = findEntryPoint(newDataFromServer);
        const newScreen = newDataFromServer.application.elements[entryPointKey];
...
export const manageActions = (actionData, id, actionHandlers, updateAppState) => {
    if (!actionData || !actionData.proc) return;

    const innerAction = actionData.proc;
    const actionHandler = actionHandlers[innerAction];
    if (actionHandler) {
        actionHandler(actionData.props || id, updateAppState); 1
    }
};
